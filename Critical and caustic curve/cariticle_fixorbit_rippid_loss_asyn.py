...
start_time = chrip_time - (scale/2)
print(f"start time : {start_time}")
end_time = start_time+scale
time = np.arange(eps,scale*tE, 0.1*tE) #เท่าของ tE
print(f"time: {time}")
...
for t in time:
    ...
    if not(isinstance(a_t, complex)) and a_t > 0:
        #++++++++++++++++++++++++++++++++++++++++++++++++++++++
        r_valuse = a_t / (1 + e * np.cos(omega*(t+start_time))) # This updates r_valuse based on that omega and time
...
r1 = M / (M + m) * r_list
r2 = -1 * (m / (M + m)) * r_list
theta_values = omega_list*(time[:len(omega_list)]+start_time)
...
for i in range(len(star_1)):
    ...
    plt.arrow(x_source[0], b_source_a, x_source[-1]-x_source[0],0,  width=0.05, head_width=0.2, head_length=0.3, fc='orange', ec='orange')
    plt.arrow(x_source[0], b_source_b, x_source[-1]-x_source[0],0,  width=0.05, head_width=0.2, head_length=0.3, fc='g', ec='g')
    plt.arrow(x_source[0], b_source_c, x_source[-1]-x_source[0],0,  width=0.05, head_width=0.2, head_length=0.3, fc='m', ec='m')
    ...
    plt.grid(True)
    #-(end_time/2)
    plt.savefig(f"D:\\blackhole_program\\graphmag\\CC_GW_fix\\{(time[i]/tE):.4f}_{M}{m}.png")
