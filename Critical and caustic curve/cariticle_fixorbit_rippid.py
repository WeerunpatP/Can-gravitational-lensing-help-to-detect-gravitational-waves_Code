...
# Parameters for lens
time = np.arange(0,4.1*tE, tE*0.1)
#print(time)
...
for i in range(len(star_1)):
    ...
    plt.title(f"Critical and Cuastic Curve of q: {q:.2f} system at tE = {time[i]/tE:.2f}")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.gca().set_aspect("equal")
    plt.grid(True)
    plt.savefig(f"C:\\Users\\LeZen_e595\\Desktop\\blackhole_program\\graphmag\\CC_GW_fix\\CC_noloss_at_{time[i]/tE:.4f}_3629.png")
    print(f"save {time[i]}")
    # plt.show()
    n += 1
    plt.close()