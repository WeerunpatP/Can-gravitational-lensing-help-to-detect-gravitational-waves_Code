...
# Parameters for lens
end_time = 4
time = np.arange(0,end_time*tE, tE*0.1)
#print(time)
# thata_E1 = ((4 * g * (M[i]*M_solar) * Dls) / ((c**2) * Ds * Dl))**(1 / 2)
# thata_E2 = ((4 * g * (m*M_solar) * Dls) / ((c**2) * Ds * Dl))**(1 / 2)
# thata_E = math.sqrt((Dls/(Ds*Dl))*((4*G*M_A)/(c**2)))
# Re = thata_E*Dl
# Re = Vt*tE

# dl = symbols('x')
# Dl_equation = Eq(sqrt((4*G*M_A*dl*(Ds-dl))/((c**2)*Ds)), Re)
# Dl = solve(Dl_equation, dl)
# Dl = Dl[0]
...
x2 = 0
y2 = 0
source_i_position = -1*(end_time/2)
x_max = 5
x_min = -5
y_max = 5
y_min = -5
...
for i in range(len(star_1)):
    ...
    plt.plot(x_source,y_source_a,color='r')
    plt.plot(x_source,y_source_b,color='g')
    plt.plot(x_source,y_source_c,color='b')

    #Plot critical + caustic
    plt.contour(X, Y, dwz_magnitude, levels=[1], colors='blue')
    plt.title(f"Critical and Cuastic Curve of q: {q:.2f} energy loss system at tE = {time[i]/tE:.2f}")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.gca().set_aspect("equal")
    plt.grid(True)
    plt.savefig(f"C:\\Users\\LeZen_e595\\Desktop\\blackhole_program\\graphmag\\CC_GW_fix_loss\\CC_noloss_at_{time[i]/tE:.4f}_3629.png")
